...
| 실험 | 핵심 수치 |
|---|---|
| 실험 A — 단일 시스템 (상승 전이 30회 + 역전이 포함 확장 40회) | 전이가 실제로 일어난 회차 전부(각 10/10)에서 서버 응답·저장 상태 무변화 (`signCount` 제외) |
| 실험 B — 라이브러리 11개 | BE/BS 노출 9/11 · 보관값과의 대조·갱신까지 구현 4/11 |
| 실험 B — 응용 3개 (Spring Security·Keycloak·필자 시스템) | 신호를 끝까지 지킨 곳 0/3 — 그중 2개는 대조·갱신을 구현한 라이브러리(webauthn4j) 위에서도 소실 |
...
5. **논의** — 위협모델에서의 의미 / 사각지대를 메우는 비용과 **적응형 인증(5.2절)** / 대안의 부재 / **이 연구로 말할 수 없는 것(5.4절)** / 검증 설계 메모 / **선행연구와의 관계(5.6절)**
6. **결론**
- 참고문헌 · 부록 A(재현 방법) · 부록 B(AI와 나의 판단) · 부록 C(문헌 재확인 기록과 후속 과제 예비 확인)

표 23개(표 1~23)와 그림 2개(그림 1~2)를 본문과 부록에 붙였다.
...
**이 연구가 새로 주장하는 것**
- 백업 상태 전이가 실제 배포된 RP에서 **관측되지 않음**을 반복 측정으로 실증한다(4.1절). **이 질문은 선행연구가 다룬 질문과 이웃해 있으나 같지 않다.** Jannett 등[19]은 103개 RP를 대상으로 BE/BS를 시험했는데, 그 시험은 **규격을 위반하는 플래그 조합을 RP가 거부하는가**를 본다 — BE=0인데 BS=1인 불가능한 조합(§7.1.17·§7.2.18)과, 등록 때와 인증 때 BE가 뒤바뀌는 경우(§7.2.19)다. 본 연구가 묻는 것은 반대쪽이다 — **규격상 정상적인 상태 변화**(BE=1, BS 0→1 및 1→0)를 RP가 기록하는가. 전자는 공격 입력에 대한 거부 여부이고, 후자는 정상 입력에 대한 관측 여부다(자세한 대조는 5.6절).
...
| Jannett 등[19] | 872개 사이트 조사와 103개 정밀 평가 — 이 연구의 **방법론적 선례이자, BE/BS 처리 실태의 대규모 대조군**이다. 이 연구의 국소적 측정이 그들의 대규모 결과와 어떻게 맞물리는지는 5.6절에서 다룬다. |
...
| 2차 조사 단계 (2026-09-11) | **실험 B의 표본을 넓히고 층위를 나눔.** 라이브러리를 10개 → **11개**로(.NET·Ruby·PHP 대안 구현 3건 추가, Spring Security·Keycloak은 라이브러리가 아니라 webauthn4j를 쓰는 **응용**이므로 이동), 그리고 **응용 층위 3개**(Spring Security·Keycloak·필자 시스템)를 새 조사축으로 신설 | 판정 근거를 소스로 끌어올린 결과 "라이브러리 층위는 규격을 반영했다"가 분명해지면서, 질문이 "그 라이브러리를 쓰는 응용은 어떤가"로 옮겨갔다(4.2.2절). 마침 기존 표본 둘이 같은 라이브러리(webauthn4j) 위에 있어 직접 비교가 가능했다. 표본 확대는 새 생태계(.NET·Ruby)를 넣어 "언어가 설명 변수가 아니다"(4.4절)를 같은 생태계 내부 대비로 확인하기 위한 것이다. 설계서 §3의 표본 수 "10개"는 사전 확정값 그대로 두고 여기에 변경을 적었다 |
...
1. **언어·생태계의 차이인가?** — 데이터로 확인 결과 **아니다.** PHP 안에서 lbuchs는 이름 붙인 접근자를 제공하고 web-auth는 제공하지 않는다 — 같은 언어, 같은 층위(라이브러리)인데 갈렸다. Java 쪽 대비(webauthn4j·Yubico는 구현, Keycloak은 안 함)도 같은 방향을 가리키지만, 이건 라이브러리와 그 라이브러리를 쓰는 응용을 나란히 놓은 것이라 층위가 다른 비교다 — 응용 층위가 신호를 잃는 원인은 4.2.2절에서 따로 다룬다. **엄밀하게 같은 층위끼리 대비한 사례는 PHP 쪽이며, 그 안에서도 언어는 설명 변수가 아니다.** 2차 조사로 표본이 늘면서 같은 생태계 내부의 대비가 두 쌍으로 늘었다.
...
**그래서 이 신호의 쓰임새는 차단이 아니라 적응이다.** BS 전이를 근거로 로그인을 막는 정책은 두 가지 이유로 권할 것이 못 된다. 첫째, 전이는 위반이 아니라 정상 동작이며 사용자가 동기화를 켜는 흔한 행동이다. 둘째, 5.1절에서 보았듯 BE/BS는 자기 신고 값이므로 차단의 근거로 삼기에는 신뢰도가 모자란다. 대신 이 신호가 어울리는 자리는 **위험 수준에 따라 요구 조건을 조절하는 적응형 인증**이다 — 평소 로그인은 그대로 통과시키되, BS가 막 전이한 자격증명으로 **되돌릴 수 없는 동작**(자격증명 삭제, 결제 수단 변경, 민감 자료 내려받기)을 시도할 때만 한 단계 더 요구하는 식이다. 이렇게 하면 정상 사용자가 치르는 비용은 거의 없고, 신호의 신뢰도가 낮다는 점도 감당 가능한 범위에 머문다 — 잘못 올린 경우의 대가가 "재확인 한 번"이기 때문이다. **관측 가능성과 방어 사이의 거리를 메우는 것은 차단 규칙이 아니라 이런 단계적 대응이다.**
...
- **실험 B는 정적 조사다.** 소스에 대조·갱신 코드가 있다는 것이 "그 라이브러리를 쓴 서비스가 실제로 그렇게 동작한다"를 뜻하지는 않는다. Spring Security·Keycloak 사례가 바로 그 간극을 보여 준다. 실제 배포된 서비스에서의 분포는 후속 연구의 몫이다.
  ...
  **이 한계를 닫는 방법은 이미 이 연구 안에 있다.** 4.2.2절의 판정 — Spring Security는 보관값을 webauthn4j에 넘기지 않아 `BEFlagVerifier`가 무동작으로 통과하고, Keycloak은 `AuthenticatorImpl`을 써서 같은 로직을 우회한다 — 은 소스를 읽어 얻은 추론이며, **실험 A와 같은 방식으로 직접 확인할 수 있다.** 두 프레임워크의 기본 통합 예제를 띄우고 시나리오 3·4를 그대로 돌려, 전이 전후의 저장 레코드(Spring Security라면 `backup_state` 컬럼)가 실제로 바뀌지 않는지 관측하면 된다. 측정 스크립트는 대상 주소만 바꾸면 그대로 쓸 수 있으므로(부록 A.2), 이 **동적 교차 검증**이 정적 조사의 열람 범위 한계를 메우는 가장 짧은 후속 과제다. 본 연구는 이를 수행하지 않았다.
...
- **응용 층위 표본 3개 중 2개가 같은 라이브러리(webauthn4j) 위에 있다.** Spring Security와 Keycloak이 공유하는 것은 "라이브러리와 응용 사이에서 신호가 끊긴다"는 문제의식만이 아니라, 그 라이브러리의 호출 방식 — 보관값을 호출자가 직접 만들어 넘겨야 대조·갱신이 작동하는 구조 — 이기도 하다. 그래서 이 두 사례가 보여주는 실패 양상이 **응용 일반의 문제**인지, **webauthn4j처럼 "호출자가 챙겨야 하는" API 설계와 결합했을 때 특히 잘 드러나는 문제**인지는 이 표본만으로는 완전히 갈라내지 못한다. go-webauthn·webauthn-rs처럼 라이브러리가 스스로 갱신하는 쪽 위에 선 실제 배포 응용을 대조군으로 조사하면 이 둘을 분리할 수 있다 — 본 연구는 이를 수행하지 않았다.
...
## 5.5 검증 설계에 대한 메모 — 가설의 형식과 기각 조건

4.5절에서 다룬 내용을 한 문장으로 남겨 둔다. **전칭 명제("~한 구현에서는 모두")의 검증은 빈도 통계가 아니라 존재 조건으로 판정된다** — 반례 하나면 충분하기 때문이다. 반대로 빈도 조건("다수가 ~한다")은 부분 긍정 명제("대체로 ~하다")에 짝이 맞는다. 이 연구의 기각 조건 R4는 후자의 형식이었고 가설은 전자의 형식이었다. 결과적으로 가설은 데이터에 근거해 정당하게 기각되었으나(4.3절), 사전에 적어 둔 조건이 그 기각을 미리 지목하지는 못했다.

가설을 세우는 단계에서 **"이 문장은 전칭인가 부분 긍정인가"를 먼저 확인하고 거기에 맞는 형식의 기각 조건을 붙이는 것**이, 이 연구를 이어받는 사람에게 남길 수 있는 가장 실용적인 조언이다.

## 5.6 선행연구와의 관계 — 무엇이 겹치고 무엇이 다른가

초고 단계에서는 Jannett 등[19]이 BE/BS를 다뤘는지 확인하지 못해 신규성 주장을 유보해 두었다. 제출 직전 아티팩트 저장소와 논문 본문을 열람해 확인한 결과, **그들은 BE/BS를 실제로 다루었다.** 103개 RP를 대상으로 한 Table 2에 두 항목이 있다.

**표 18 — Jannett 등[19]의 BE/BS 시험과 본 연구의 질문**

| | Jannett 등[19] (103개 RP) | 본 연구 |
|---|---|---|
| 무엇을 보내는가 | **규격을 위반하는 플래그** | **규격상 정상인 플래그** |
| Backup State 시험 | BE=0인데 BS=1인 불가능한 조합을 보냄(§7.1.17·§7.2.18). 거부하지 못한 곳 **65개** | — |
| Backup Eligible 시험 | 등록 때와 인증 때 BE를 뒤바꿔 보냄(§7.2.19). 거부하지 못한 곳 **57개** | — |
| 본 연구의 시험 | — | BE=1을 유지한 채 BS를 0↔1로 전이시키고, RP의 응답·보관 레코드가 달라지는지 관측 |
| 묻는 것 | RP가 **공격 입력을 거부**하는가 | RP가 **정상적인 상태 변화를 기록**하는가 |
| 측정 방식 | 103개 RP 대상 정적·동적 대규모 시험 | 단일 RP 반복 측정 + 라이브러리·응용 소스 조사 |

**두 결과는 서로를 보강한다.** 그들의 "Backup Eligible" 시험은 규격 §7.2.19의 대조 단계 — 현재 BE를 **보관된 BE와 비교하라** — 를 그대로 시험한 것이다. 103개 중 57개가 이 뒤바꿈을 거부하지 못했다는 것은, **그 RP들이 대조할 보관값을 갖고 있지 않다는 뜻**이다. 본 연구가 단일 시스템에서 소스 수준으로 확인한 것(보관 컬럼 자체가 없다)과 4.2.2절에서 Spring Security·Keycloak에 대해 확인한 것이, 그들의 규모에서 같은 방향으로 나타난 셈이다.

**동시에 두 연구가 답하지 않는 영역도 분명해진다.** 그들의 시험은 공격자가 보낸 **위반 조합**에 대한 반응을 본다. 정상적인 BS 전이는 위반이 아니므로 그 시험에 걸리지 않으며, 따라서 "RP가 정상적인 상태 변화를 기록하는가"는 그들의 103개 표본에서도 여전히 답해지지 않았다. 본 연구가 채우는 자리가 거기다 — 다만 그 자리를 채운 표본은 RP 하나이므로, **두 연구를 합쳐도 "정상 전이를 기록하는 RP가 얼마나 되는가"라는 분포는 아직 비어 있다.** 그들의 PASSKEYS-ATTACKER에 정상 전이 시나리오를 하나 더하는 것이 이 공백을 메우는 가장 짧은 경로로 보인다.
...
이 결과는 선행연구와 이웃해 있으나 겹치지 않는다. Jannett 등[19]은 103개 RP에 **규격을 위반하는 플래그 조합**을 보내 거부 여부를 시험했고, 두 항목에서 각각 65곳과 57곳이 걸러내지 못했다. 본 연구는 반대쪽을 본다 — **규격상 정상인 상태 변화**를 RP가 기록하는가. 그들의 결과는 대조할 보관값이 없는 RP가 널리 존재함을 대규모로 보여 주고, 본 연구는 그 보관값이 왜 없는지를 소스 수준에서 따라간다(5.6절).

실무적 함의는 네 가지다. 첫째, 이 사각지대를 닫는 비용은 컬럼 두 개와 코드 몇 행이며, 반례들이 이미 그렇게 하고 있다는 사실이 그 현실성을 보증한다. 그리고 **라이브러리를 쓰는 쪽이라면 확인할 것은 세 단계다** — 값을 저장 모델에 담는가, 인증 때 그 값을 라이브러리에 다시 넘기는가, 갱신된 값을 받아 저장하는가. Spring Security는 두 번째에서, Keycloak은 첫 번째에서, 필자의 시스템은 첫 번째부터 빠져 있었다. 라이브러리가 알아서 해 줄 것이라는 가정이 가장 위험하다 — 그 라이브러리가 실제로 해 주고 있어도 호출하는 쪽이 닿지 않으면 소용이 없다. 둘째, 그럼에도 이것은 방어가 아니라 **관측**이다. 기기 고유 키로 기기를 식별하려던 표준화 시도들이 2024년에 모두 접힌 지금, RP가 할 수 있는 가장 실효적인 일은 이미 매 응답에 실려 오는 신호를 읽는 것이다. 셋째, 그러므로 이 신호가 어울리는 자리는 차단이 아니라 **적응형 인증**이다 — 평소 로그인은 그대로 두고, 막 전이한 자격증명으로 되돌릴 수 없는 동작을 시도할 때만 한 단계 더 요구하는 식이다(5.2절). 전이는 위반이 아니라 정상 동작이고, 자기 신고 값이라 차단의 근거로 삼기에는 신뢰도가 모자라기 때문이다. 넷째, 그 신호는 정직한 인증장치의 상태 변화를 놓치지 않기 위한 것이지 위장하는 공격자를 잡아내는 수단이 아니다(5.1절) — 이 네 가지를 구분하지 못하면, 신호를 기록하기 시작하는 것이 오히려 잘못된 안심을 낳을 수 있다. 무엇이 바뀌었는지조차 볼 수 없다면 무엇을 지킬지 정할 수도 없지만, 본다고 해서 지켜지는 것도 아니다.
...
| `논문.md` | 본 논문 (표 23개 · 그림 2개 · 참고문헌 21건 · 부록 A~C) |
...
| `원자료-구현체조사.md` | 실험 B 원자료 — 1차 조사(라이브러리 10개)와 2차 조사(판정 3건 갱신·신규 3건·응용 층위 3개), 근거 인용문과 확인 출처, **채택 규모** |
...
ZIP 하나에 위 9개 파일이 평평하게 들어 있다. 원자료의 건수와 설계서의 표본 수는 다음과 같이 대응한다 — **실험 A**는 설계서대로 시나리오 3종 × 10회 = 30회이고(원자료 `집계.총회차` = 10, 회차마다 3종 실행), 여기에 수집 종료 후 확장한 4종 × 10회 = 40회가 별도 파일로 있다(3.5절). **실험 B**는 설계서의 목표치가 라이브러리 10개였고, 2차 조사에서 라이브러리 11개와 응용 3개로 넓혔다(3.5절). 설계서 §3의 "10개"는 사전 확정 시점의 값 그대로 두고, 변경은 설계서 §5와 3.5절에 기록했다.

**공개 배포본 대조의 재현 한계.** 3.2절의 보조 확인(②·③)은 `https://aleph-passkey.vercel.app`이 이 논문 작성 시점과 같은 코드를 서비스하고 있다는 전제 위에 있다. 저장소가 이후 갱신되면 배포본도 함께 바뀌어 그 대조는 재현되지 않을 수 있다 — 로컬 서버(①·②)로 재현하는 경로는 저장소 자체를 그대로 쓰므로 이 문제에서 자유롭다. 배포본까지 같은 시점으로 맞추려면, 원자료의 `측정시각`(2026-09-10 06:19 UTC 시작)을 기준으로 저장소 커밋 로그에서 그 시각 직전 커밋을 찾아(`git log --until="2026-09-10T06:19:00Z"`) 그 커밋을 로컬에서 띄우면 된다.
...
**표 21 — 제출 직전 문헌 재확인 기록 (2026-09-11)**
...
**표 22 — 후속 과제(BS 1→0)의 측정 가능성 예비 확인 (2026-09-11)**
...
## 추가 확인 — 선행연구 [19]의 BE/BS 취급과 §7.2 단계 번호

초고 단계에서 유보해 두었던 두 가지를 제출 직전에 확인했다.

**표 23 — 제출 직전 추가 확인: 선행연구의 BE/BS 취급과 규격 단계 번호**

| 항목 | 확인 방법 | 확인된 것 |
|---|---|---|
| Jannett 등[19]이 BE/BS를 다뤘는가 | 아티팩트 저장소(`RUB-NDS/state-of-passkeys-artifacts`)를 내려받아 전수 검색하고, 함께 배포된 논문 본문 Table 2를 열람 | **다루었다.** "Backup State"(BE=0인데 BS=1인 조합을 보내는 시험, 거부하지 못한 곳 65개)와 "Backup Eligible"(등록·인증 사이에 BE를 뒤바꾸는 시험, 57개) 두 항목이 103개 RP 평가에 포함되어 있다. 두 항목 모두 선행 연구 참조가 없는 신규 시험으로 표시되어 있다 |
| 규격 §7.2의 BE/BS 검증 단계 번호 | 위 Table 2의 규격 참조 표기와, webauthn4j `BEFlagVerifier` Javadoc | 서로 독립된 두 출처가 같은 번호를 지목한다 — BE=0⇒BS=0 검증은 **§7.1.17·§7.2.18**, 보관된 BE와의 대조는 **§7.2.19**. 규격 원문에서 번호를 직접 세지는 못했으므로 본문에는 두 출처를 함께 밝혀 적었다(2.3절) |

**이 확인이 논문에 미친 영향.** 1.5절의 신규성 주장과 5.6절이 새로 쓰였다. 두 연구가 겹치지 않는 지점 — 그들은 **규격을 위반하는 플래그**에 대한 거부 여부를, 본 연구는 **규격상 정상인 상태 변화**에 대한 기록 여부를 본다 — 이 명확해졌고, 동시에 그들의 대규모 결과가 본 연구의 소스 수준 관찰과 같은 방향임을 확인할 수 있게 되었다. 참고문헌 [1]에는 §4·§6.1.3·§7.2의 직접 앵커 링크를 그대로 두어 인용 문장을 바로 찾을 수 있게 했다.
