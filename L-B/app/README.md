...
**이 앱은 패스키 로그인을 만드는 개발자가 로그인 응답을 넣으면, 논문이 확인한 규칙(규격 §7.2 단계 18·19와 적응형 인증)으로 백업 상태(BE/BS) 전이를 판정하고 서버에 빠진 저장·대조 단계를 알려 줍니다.**
...
| # | 할 일 | 해 보는 방법 | 나오는 것 |
|---|---|---|---|
| 1 | 로그인 응답에서 BE·BS 읽기 | 01 칸에 `authenticatorData`(base64url·16진수)나 플래그 한 바이트(`0x1d` 등)를 넣거나 예시 버튼을 누름 | 비트 8개와 BE·BS 뜻 |
| 2 | 보관한 값과 비교해 대응 받기 | 02 칸에서 서버가 저장한 BE·BS·"한 번이라도 백업된 적"과 지금 하려는 동작을 고름 | **통과 / 한 단계 더 확인 / 거부**, 근거(논문 절), 이번 로그인 뒤 저장할 값 |
| 3 | 내 서버의 빠진 단계 찾기 | 03 칸에서 쓰는 라이브러리·응용을 고르고 세 질문에 답함 | 논문 표 14의 판정, 빠진 단계, 고치는 SQL·JS |
