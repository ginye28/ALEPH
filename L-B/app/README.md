...
- 위쪽 탭(대시보드·점검·서버 진단·기록)으로 화면을 옮겨 다닙니다 — 스크롤로 쭉 읽는 한 페이지가 아니라, 필요한 화면으로 바로 이동합니다.
- **대시보드**가 시작 화면입니다. 처음 열면 짧은 소개와 "예시로 먼저 해보기"가 뜨고, 두 번째부터는 지금까지 몇 번 점검했는지·마지막 판정·최근 기록이 먼저 보입니다 — 매번 새로 시작하는 도구가 아니라 다시 열어도 내가 뭘 했는지 남아 있는 도구입니다.
...
- 판정이 나올 때마다 이 브라우저의 **기록**에 자동으로 쌓입니다(같은 값을 반복 입력해도 다시 쌓이지 않음). "기록" 탭이나 대시보드의 최근 기록에서 "불러오기"를 누르면 그 시점의 입력·판정·스택 선택이 그대로 되돌아옵니다.
- Step 3에 라이브러리 11개·응용 3개 전체를 한 화면에서 보는 비교표가 있습니다(내 스택 하나가 아니라 생태계 전체를 참고 자료로 볼 때).
- 지금 화면 상태(입력값·판정·스택 선택·탭)를 담은 링크를 복사해 공유할 수 있고, Step 1~3 결과를 텍스트 리포트로도 복사할 수 있습니다 — 둘 다 이 브라우저 안에서만 만들어집니다.
- 시스템이 다크 모드면 이 페이지도 자동으로 어두워집니다. 오른쪽 위 해·달 아이콘을 누르면 시스템 설정과 상관없이 라이트/다크로 고정할 수 있고(순서: 시스템→라이트→다크→시스템), 고른 값은 다음에 열 때도 그대로 기억됩니다.
...
- 점검 기록은 `localStorage`(`bscheck:history`, 최근 30건)에 시각·판정·플래그·스택 선택만 이 브라우저에 남습니다. "기록" 탭의 "기록 전부 지우기"로 언제든 지울 수 있고, 서버로는 전송되지 않습니다.
- 처음 방문했는지는 `localStorage`(`bscheck:visited`)로만 구분합니다 — 개인 식별 값이 아니라 "안내 카드를 봤는지" 표시입니다.
- 라이트/다크 선택도 `localStorage`(`bscheck:theme`)에 "light"·"dark" 둘 중 하나로만 남습니다. "시스템"으로 되돌리면 이 값도 지워집니다.
