// BS Check — 화면 연결. 판정 규칙은 logic.js에 있다.
(function () {
  'use strict';
  const B = window.BSCheck;
  const $ = (id) => document.getElementById(id);
  const esc = (s) => String(s).replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
  const icon = (id) => `<svg class="icon ic"><use href="#i-${id}"/></svg>`;
  const VERDICT_ICON = { pass: 'check-circle', stepup: 'alert-triangle', reject: 'x-circle' };
  const LEVEL_ICON = { ok: 'check-circle', warn: 'alert-triangle', bad: 'x-circle', info: 'info' };
  ...
  function renderRead() {
    ...
    if (!r.ok) {
      parsed = null;
      out.innerHTML = `<p class="err" role="alert">${icon('x-circle')}${esc(r.error)}</p>`;
      return;
    }
    parsed = r;
    ...
    const cells = B.FLAG_BITS.slice().reverse().map((b) => {
      ...
      const ic = b.name === 'BE' ? `<div class="ic"><svg class="icon"><use href="#i-lock"/></svg></div>` : b.name === 'BS' ? `<div class="ic"><svg class="icon"><use href="#i-cloud"/></svg></div>` : '';
      return `<div class="bit${on ? ' on' : ''}${key ? ' key' : ''}">${ic}<div class="nm">${b.name}</div><div class="v">${on ? 1 : 0}</div><div class="mk">${B.hex2(b.mask)}</div></div>`;
    }).join('');
    ...
    out.innerHTML = `
      ...
      <p class="hint">이 값만으로는 “방금 바뀌었는지”를 알 수 없습니다. Step 2에서 서버가 보관한 값과 비교합니다.</p>`;
  }
  ...
  function renderJudge() {
    const out = $('judge-out');
    if (!parsed) {
      out.innerHTML = '<p class="meta">Step 1에 로그인 응답을 넣으면 판정이 나옵니다.</p>';
      return;
    }
    ...
    const findings = r.findings.map((x) => `
      <div class="finding ${x.level}">${icon(LEVEL_ICON[x.level])}<div><h3>${esc(x.title)}</h3><p>${esc(x.body)}</p><span class="ref">${esc(x.ref)}</span></div></div>`).join('');
    ...
    out.innerHTML = `
      <div class="verdict ${r.verdict}"><svg class="icon ic"><use href="#i-${VERDICT_ICON[r.verdict]}"/></svg><div><span class="badge">${v.label}</span><p>${esc(v.body)}</p></div></div>
      ${findings}
      <div class="rowhead"><h3>이번 로그인 뒤 저장할 값</h3><button type="button" class="copy" id="copy-next">${icon('copy')}복사</button></div>
      <pre id="next-values">${esc(next)}</pre>`;
    $('copy-next').addEventListener('click', (e) => copyText(e.currentTarget, next));
  }
  ...
  function fillLibs() {
    ...
    $('lib').innerHTML = `
      ...
      <optgroup label="라이브러리">${libs}</optgroup>
      <optgroup label="응용">${apps}</optgroup>
  ...
  function renderLibInfo() {
    const v = $('lib').value;
    const info = $('lib-info');
    document.querySelectorAll('#stack-picks button').forEach((b) => b.classList.toggle('active', b.dataset.lib === v));
    if (!v) { info.innerHTML = ''; return; }
    const [kind, id] = v.split(':');
    if (kind === 'app') {
      const a = B.APPS.find((x) => x.id === id);
      const lib = B.LIBRARIES.find((x) => x.id === a.lib);
      info.innerHTML = `<p class="note">${icon('info')}<span><b>${esc(a.name)}</b> — 기반 라이브러리 ${esc(lib.name)}. ${esc(a.note)} 아래 답은 조사한 판정으로 채웠습니다.</span></p>`;
    ...
    const row = lib
      ...
      : '';
    info.innerHTML = `${row}<p class="note">${icon('info')}<span>${esc(B.LIB_NOTES[id] || B.LIB_NOTES.other)}</span></p>`;
  }
  ...
  function renderAudit() {
    ...
    let head;
    if (r.status === 'ok') head = `<div class="verdict pass">${icon('check-circle')}<div><span class="badge">세 단계 모두 있음</span><p>BE/BS 신호가 응용까지 이어집니다. Step 2의 판정 규칙을 정책에 붙이면 됩니다.</p></div></div>`;
    else if (r.status === 'gap') head = `<div class="verdict reject">${icon('x-circle')}<div><span class="badge">${r.missing.map(n).join('·')}단계 빠짐</span><p>신호는 ${n(r.firstGap)}단계에서 끊깁니다. 이 상태에서는 전이가 일어나도 서버에 아무 변화가 없습니다.</p></div></div>`;
    else head = `<div class="verdict stepup">${icon('alert-triangle')}<div><span class="badge">확인 필요</span><p>${r.unknown.map(n).join('·')}단계를 모릅니다. 저장소 스키마와 로그인 검증 코드에서 BE·BS(backupEligible·backupState)를 찾아보세요.</p></div></div>`;
    out.innerHTML = `<div class="audit-result">${head}
      ${r.status === 'ok' ? '' : `
      <div class="rowhead"><h3>1. 저장 모델에 컬럼 추가</h3><button type="button" class="copy" data-copy="sql">${icon('copy')}복사</button></div><pre id="fix-sql">${esc(B.FIX_SQL)}</pre>
      <div class="rowhead"><h3>2·3. 로그인 때 대조하고 갱신</h3><button type="button" class="copy" data-copy="js">${icon('copy')}복사</button></div><pre id="fix-js">${esc(B.FIX_JS)}</pre>
      <p class="hint">추가 왕복도 사용자 상호작용도 필요 없고, 컬럼 세 개와 코드 몇 줄이면 닫힙니다.</p>`}
    </div>`;
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('stack-picks').addEventListener('click', (e) => {
      const b = e.target.closest('button[data-lib]');
      if (!b) return;
      $('lib').value = b.dataset.lib;
      renderLibInfo();
      renderAudit();
    });
    $('questions').addEventListener('change', renderAudit);
    $('audit-out').addEventListener('click', (e) => {
      const b = e.target.closest('button[data-copy]');
      if (!b) return;
      copyText(b, b.dataset.copy === 'sql' ? B.FIX_SQL : B.FIX_JS);
    });
  });
})();
