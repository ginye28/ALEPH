...
const PATHS = {
  ...
  sun: <><circle cx="12" cy="12" r="4.3" /><path d="M12 2.8v2.6M12 18.6v2.6M4.6 4.6l1.8 1.8M17.6 17.6l1.8 1.8M2.8 12h2.6M18.6 12h2.6M4.6 19.4l1.8-1.8M17.6 6.4l1.8-1.8" /></>,
...
function ThemeToggle() {
  const [theme, setTheme] = useState(() => {
    try { const t = document.documentElement.getAttribute('data-theme'); return t === 'light' || t === 'dark' ? t : 'system'; }
    catch (e) { return 'system'; }
  });
  useEffect(() => {
    try {
      if (theme === 'system') { document.documentElement.removeAttribute('data-theme'); localStorage.removeItem('bscheck:theme'); }
      else { document.documentElement.setAttribute('data-theme', theme); localStorage.setItem('bscheck:theme', theme); }
    } catch (e) { /* 사생활 보호 모드 등 — 이번 방문에서만 적용됨 */ }
  }, [theme]);
  const next = { system: 'light', light: 'dark', dark: 'system' };
  const icon = { system: 'auto', light: 'sun', dark: 'moon' }[theme];
  const label = { system: '시스템 설정을 따르는 중', light: '라이트 모드로 고정됨', dark: '다크 모드로 고정됨' }[theme];
  return (
    <button type="button" onClick={() => setTheme(next[theme])} title={`${label} — 눌러서 바꾸기`} aria-label={`테마: ${label}. 눌러서 바꾸기`}
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-hair/60 bg-white/80 backdrop-blur-xl">
      <div className="mx-auto flex h-12 max-w-[980px] items-center justify-between px-6 text-[12px]">
        <button type="button" onClick={() => setView('dashboard')} className="font-semibold tracking-tight text-ink">BS Check</button>
  ...
  return (
    <section className="bg-mist" aria-labelledby="h-proof">
      <div className="mx-auto max-w-[980px] px-6 py-32 sm:py-40">
        <Fade as="h2" id="h-proof" className="text-center text-[14px] font-semibold text-sub">왜 이 점검이 필요한가</Fade>
        <Fade as="p" className="mx-auto mt-3 max-w-[440px] text-center text-[12.5px] leading-[1.6] text-sub">
          저자가 직접 측정·조사한 결과 · <a href={PAPER_URL} target="_blank" rel="noopener" className="underline underline-offset-4 hover:text-ink">연구 논문에서 방법 보기</a>
        </Fade>
...
function saveHistory(list) { safeSetJSON(HISTORY_KEY, list.slice(0, MAX_HISTORY)); }
function pushHistory(entry) {
  const id = Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
  const next = [{ id, at: new Date().toISOString(), ...entry }, ...loadHistory()];
  saveHistory(next);
  return next;
}
...
function PolicyStep({ parsed, stored, setStored, action, setAction, result }) {
...
function CheckHeader() {
  return (
    <header className="bg-white">
      <div className="mx-auto max-w-[980px] px-6 pt-16 pb-2">
        <Fade as="p" className="text-[13px] font-semibold text-sub">점검</Fade>
        <Fade as="h1" className="mt-3 text-[32px] font-extralight leading-[1.15] tracking-[-0.02em] sm:text-[42px]">로그인 응답을 넣어 백업 상태 전이를 판정합니다.</Fade>
      </div>
    </header>
  );
}
...
// ── 기록 ────────────────────────────────────────────────
function HistoryRow({ entry, onRestore }) {
  ...
  return (
    <li className="flex items-center justify-between gap-4 py-5">
      <div className="min-w-0">
        <p className="truncate text-[16px]">{vt ? vt.label : entry.verdict}</p>
        <p className="mt-1 text-[12.5px] text-sub">{fmtWhen(entry.at)} · 플래그 {entry.flagsHex} · {src}</p>
      </div>
      <button type="button" onClick={() => onRestore(entry)} className="shrink-0 text-[13px] text-sub hover:text-ink underline underline-offset-4">불러오기</button>
    </li>
  );
}

function HistoryView({ historyList, onRestore, onClear }) {
  return (
    <Section id="history" eyebrow="기록" title="지난 점검들."
      lead="이 브라우저에 남아 있는 점검 기록입니다. 눌러서 그 상태로 다시 불러올 수 있습니다 — 서버에는 애초에 저장되지 않습니다.">
      {historyList.length === 0 ? (
        <p className="text-[15px] text-sub">아직 기록이 없습니다. 점검을 하면 여기 쌓입니다.</p>
      ) : (
        <>
          <ul className="divide-y divide-hair">{historyList.map((h) => <HistoryRow key={h.id} entry={h} onRestore={onRestore} />)}</ul>
          <button type="button" onClick={onClear} className="mt-8 text-[13px] text-sub hover:text-ink underline underline-offset-4">기록 전부 지우기</button>
        </>
      )}
    </Section>
  );
...
// ── 대시보드 ────────────────────────────────────────────
function Dashboard({ historyList, showIntro, onStartExample, onDismissIntro, onStart, onGoHistory, onRestore }) {
      ...
      {showIntro && (
        <section className="bg-white">
          <div className="mx-auto max-w-[980px] px-6 pt-20">
            <Fade className="rounded-3xl bg-mist px-8 py-10 sm:px-12 sm:py-14">
              <p className="text-[14px] font-semibold text-sub">WebAuthn · 패스키</p>
              <h1 className="mt-3 text-[28px] font-extralight leading-[1.2] tracking-[-0.02em] sm:text-[38px]">로그인 응답 하나로<br />백업 상태 전이를 판정합니다.</h1>
              <p className="mt-5 max-w-[560px] text-[15px] font-light leading-[1.6] text-sub">패스키 로그인을 만드는 개발자를 돕는 도구입니다. 계산은 전부 이 브라우저 안에서 이뤄지고, 값은 어디로도 나가지 않습니다.</p>
              <div className="mt-8 flex flex-wrap items-center gap-x-8 gap-y-4">
                <button type="button" onClick={onStartExample} className="inline-flex min-h-[44px] items-center rounded-full bg-ink px-7 text-[15px] font-medium text-white hover:bg-inkhover">예시로 먼저 해보기</button>
                <button type="button" onClick={onDismissIntro} className="text-[15px] text-sub hover:text-ink">건너뛰기</button>
              </div>
            </Fade>
          </div>
        </section>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-[980px] px-6 py-24 sm:py-32">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              <p className="text-[14px] font-semibold text-sub">지금까지</p>
              <p className="mt-2 text-[40px] font-extralight tracking-[-0.02em]">{total}번 점검함</p>
            </div>
            <button type="button" onClick={onStart} className="inline-flex min-h-[44px] items-center rounded-full bg-ink px-7 text-[15px] font-medium text-white hover:bg-inkhover">새 점검 시작</button>
          </div>
          {last && (
            <div className="mt-10 rounded-2xl bg-mist px-6 py-6">
              <p className="text-[13px] font-semibold text-sub">마지막 판정 · {fmtWhen(last.at)}</p>
              <p className="mt-2 text-[22px] font-light">{(B.VERDICT_TEXT[last.verdict] || {}).label || last.verdict}</p>
            </div>
          )}
          {total > 0 && (
            <div className="mt-10">
              <div className="flex items-center justify-between">
                <p className="text-[13px] font-semibold text-sub">최근 기록</p>
                <button type="button" onClick={onGoHistory} className="text-[13px] text-sub hover:text-ink underline underline-offset-4">전체 보기</button>
              </div>
              <ul className="mt-4 divide-y divide-hair">
                {historyList.slice(0, 5).map((h) => <HistoryRow key={h.id} entry={h} onRestore={onRestore} />)}
              </ul>
            </div>
          )}
        </div>
      ...
      <div className="mx-auto max-w-[980px] border-t border-hair px-6 py-16 text-[12px] leading-[1.8] text-sub">
        ...
        <details className="mt-8 group">
          ...
          <ul className="mt-4 max-w-[640px] space-y-3">
            ...
            <li>이 도구는 「동기화 패스키의 백업 상태 전이는 서비스에 관측되는가」(진혜정, 2026) 논문의 결과를 씁니다 — 위 "연구 논문" 링크에서 원문을 볼 수 있습니다.</li>
          </ul>
        </details>
...
function App() {
  ...
  const goView = (v) => {
    if (v !== 'dashboard') {
      try { localStorage.setItem(VISITED_KEY, '1'); } catch (e) { /* 무시 */ }
      setShowIntro(false);
    }
    setViewRaw(v);
  ...
  const startExampleFromIntro = () => { applyExample(first.key); goView('check'); };
  ...
  const clearHistory = () => { safeRemove(HISTORY_KEY); setHistoryList([]); };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setHistoryList(pushHistory({
        flagsHex: B.hex2(parsed.flags), verdict: result.verdict, transition: result.transition,
        action, pick, input, stored: { ...stored }, source: activeEx ? 'example' : 'manual',
      }));
    }, 900);
  ...
  return (
    <>
      <Nav view={view} setView={goView} />
      <main>
        {view === 'dashboard' && (
          <Dashboard historyList={historyList} showIntro={showIntro} onStartExample={startExampleFromIntro} onDismissIntro={dismissIntro}
            onStart={() => goView('check')} onGoHistory={() => goView('history')} onRestore={restoreFromHistory} />
        )}
        {view === 'check' && (
          <>
            <CheckHeader />
            ...
            <PolicyStep parsed={parsed} stored={stored} setStored={setStored} action={action} setAction={setAction} result={result} />
          </>
        )}
        {view === 'audit' && <AuditStep pick={pick} choose={choose} answers={answers} setAnswers={setAnswers} />}
        {view === 'history' && <HistoryView historyList={historyList} onRestore={restoreFromHistory} onClear={clearHistory} />}
