...
<h3>1일차에 고정한 것 (T07-C04·C05·C06)</h3>
${table(["항목", "고정한 값"], [
    ["답하려는 질문 한 문장", "계획한 시간과 실제로 쓴 시간이 얼마나 벌어지는가?"],
    ["관찰 지표 한 개", "그날 실행기록의 실제 걸린 시간 합계"],
    ["지표의 단위", "분(minute)"],
...
<p class="muted">9월 5일·6일 기록은 실제로 한 날짜·시각(각 13:00~13:30)으로 적었지만,
   도구 문제로 그날 바로 남기지 못하고 <b>9월 8일에 한꺼번에 입력</b>했습니다. 내보내기 파일의
   <code>createdAt</code>에 그 사실이 그대로 남아 있어 숨기지 않고 밝힙니다 — 공부한 시각은
   <code>startedAt</code>·<code>endedAt</code> 그대로입니다.</p>

<div class="todo">
  <b>아직 못 채운 것 — 계획 규칙 변경 (T07-C09~C15).</b> 2일차 뒤·3일차 앞에 놓여야 하는
  규칙 변경 기록을 아직 넣지 못했습니다. 이 5일에서 3일차는 9월 4일인데,
  <code>review_notes.created_at</code>은 서버가 INSERT 시점에 찍고 이 표는 RLS에
  UPDATE/DELETE 정책이 없어(append-only) 지난 시각으로 되돌릴 수 없습니다.
  <b>"넣었다"고 적지 않고 못 넣었다고 적습니다</b> — 순서를 지어내는 것보다 낫습니다.
</div>
