...
${table(["구분", "내용"], [
    ["AI에게 맡긴 일", "Supabase Auth 연동(auth.js/AuthGate/AuthForm/AccountSection), user_id + stamp_owner 트리거 + RLS 스키마 재작성, 검사 18~40 설계·구현, capture.mjs/report.mjs를 인증 흐름에 맞게 재작성, session_id·auth.sessions 기반 즉시 세션 무효화(session_is_active()) 조사·구현, 계정 삭제 버튼을 로그아웃과 시각적으로 분리(위험 구역 스타일), 새 Supabase·Vercel 프로젝트 생성 과정에서 비밀번호가 필요 없는 모든 단계."],
...
<p>Asia/Seoul 기준 <b>서로 다른 실제 날짜 정확히 5일</b>입니다. 아래 값은 내보내기 파일의
   실행기록을 손으로 더한 것이고, <code>tools/check.mjs</code>의 검사 31이 같은 계산을 다시 해
   대조합니다.</p>
...
<h4>같은 값을 화면도 직접 보여 줍니다 (T07-C132)</h4>

<p>자료 화면의 <b>"날짜별 실제 시간"</b> 표가 위와 같은 규칙으로 내 실행기록 전체를 날짜로 묶어
날짜별 값·합계·하루 평균을 그립니다(<code>src/components/DailySection/DailySection.jsx</code>,
규칙은 <code>src/core/dailyTotals.js</code>). 계획 하나 안을 보는 돌아보기와 달리 계획·할일
경계를 넘어 봅니다 — 카드 5가 묻는 것이 "며칠에 걸쳐 얼마나 했는가"이기 때문입니다.</p>

${figure("14_날짜별_합계평균", `${st("14_날짜별_합계평균")} — ${nt("14_날짜별_합계평균")}`)}

<p class="muted">위 화면은 <b>증빙용 스크래치 계정</b>에 서로 다른 날짜 5일을 심어 찍은 것입니다.
심사하는 분이 제 계정으로 로그인하지 않으므로(7.md 증거 보관 기준), 같은 표가 무엇을 그리는지를
새 계정에서 재현해 보였습니다. 제 실제 5일 값은 바로 위 표에 있습니다.
화면이 그린 숫자가 손으로 더한 값과 같은지는 검사 40이 매번 대조합니다 —
<code>${esc(checkOf(40)?.detail ?? "")}</code> ${passBadge(40)}</p>
...
<p class="muted">
  ...
  1~17은 과제 6과 같은 계획·할일·실행기록·돌아보기 검사이고, 18~40이 이번 과제(인증·소유권)에서
