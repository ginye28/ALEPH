...
// ───────────────────────────────────────── 검사 39 · 과제 6 자료가 내 계정으로 옮겨졌는지
//
// T07-C100은 "6번에 넣어 둔 내 자료가 내 계정으로 옮겨져 있다"를 요구합니다.
// 옮겼다는 문장이나 이전 코드의 존재는 근거가 아닙니다 — 과제 6에서 내보낸 파일과
// 과제 7 계정에서 내보낸 파일을 id로 맞대어, 한 줄도 빠지지 않았는지 셉니다.
// id가 그대로라는 것은 다시 입력한 것이 아니라 실제로 옮겨졌다는 뜻입니다.
await guard(39, async () => {
    const 파일들 = fs.readdirSync(ROOT);
    const T06 = 파일들.filter((f) => /^plandosee-내보내기-.*\.json$/.test(f)).sort().at(-1);
    const T07 = 파일들.filter((f) => /^plandosee-auth-내보내기-.*\.json$/.test(f)).sort().at(-1);

    if (!T06 || !T07) {
        return hold(
            39,
            `보류 — 맞대어 볼 파일이 없습니다(과제 6 내보내기 ${T06 ?? "없음"} · 과제 7 내보내기 ${T07 ?? "없음"}). ` +
                `두 파일을 저장소 맨 위에 두고 다시 돌리면 id로 맞대어 셉니다`,
        );
    }

    const 읽기 = (f) => JSON.parse(fs.readFileSync(path.join(ROOT, f), "utf-8"));
    const 이전 = 읽기(T06);
    const 이후 = 읽기(T07);

    const 표들 = ["plans", "planRevisions", "tasks", "executionRecords", "reviewNotes"];
    const 조각 = [];
    let 빠진것 = 0;
    let 옮겨온것 = 0;

    for (const 표 of 표들) {
        const 전 = new Set((이전[표] ?? []).map((r) => r.id));
        const 후 = new Set((이후[표] ?? []).map((r) => r.id));
        const 없는것 = [...전].filter((id) => !후.has(id));
        빠진것 += 없는것.length;
        옮겨온것 += 전.size - 없는것.length;
        조각.push(`${표} ${전.size}건 중 ${전.size - 없는것.length}건`);
    }

    if (빠진것 === 0 && 옮겨온것 > 0) {
        pass(
            39,
            `과제 6 내보내기(${T06})의 ${옮겨온것}건이 과제 7 계정 내보내기(${T07})에 ` +
                `id 그대로 전부 있음 — ${조각.join(" · ")} · 빠진 행 0건 ` +
                `(id가 같다는 것은 다시 입력한 것이 아니라 실제로 옮겨졌다는 뜻입니다)`,
        );
    } else {
        fail(39, `빠진 행 ${빠진것}건 — ${조각.join(" · ")}`);
    }
});
