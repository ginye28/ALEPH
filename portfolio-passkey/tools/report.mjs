...
<h2 class="section">② 왜 그걸 골랐나</h2>
<p>고른 이유는 ①에 적었습니다. 여기에는 <b>검토했지만 고르지 않은 것</b>과 그 이유를 나란히 둡니다 — 무엇을 골랐는지보다 무엇을 버렸는지가 이유를 더 분명히 보여 주기 때문입니다.</p>
${table(
    ["후보", "고르지 않은 이유"],
    [
        [
...
<h2 class="section">③ 어디를 어떻게 고쳤나</h2>
<h3>비밀번호를 어떻게 맡아 두나</h3>
...
<h3>등록·로그인·로그아웃·비공개 자료 조회가 소스 어디를 지나는가 (T08-C49)</h3>
${table(
    ["흐름", "지나는 자리", "하는 일"],
    [
        [
...
<h2 class="section">④ 안 열리는 것을 확인한 기록</h2>
<p>확인 네 가지를 <b>성공한 요청과 거절된 요청을 나란히</b> 둡니다. "막았습니다"라는 문장이 아니라 막히는 장면이 근거입니다.</p>
...
<h2 class="section">⑤ AI와 나</h2>
<p>AI에게 맡긴 일, 내가 직접 판단한 일, AI 제안을 따르지 않은 일을 나눠 적습니다 (T08-C53).</p>
<h3>AI에게 맡긴 일</h3>
...
<p><b>먼저, 이 항목에 적었다가 실제로 막은 것들입니다.</b> ⑥은 비워 두지 않는 자리이지만,
   적어 놓고 그대로 두는 자리도 아닙니다. 앞선 제출 뒤에 아래 셋을 코드로 고쳤고, 고친 것을
   말이 아니라 검사로 남겼습니다.</p>
<ul>
  <li><b>잠금이 풀린 기기</b> — 등록·로그인 모두 <code>userVerification: "required"</code>로 올렸고,
      서버도 응답의 UV 플래그를 <code>requireUserVerification: true</code>로 다시 봅니다. ${badge(36)}</li>
  <li><b>로그인만으로 패스키 삭제</b> — 되돌릴 수 없는 동작에는 5분 이내의 패스키 재확인을 요구합니다. ${badge(37)} ${badge(38)}</li>
  <li><b>다 쓴 질문이 쌓이는 것</b> — 질문을 새로 만들 때 만료된 지 한 시간이 지난 줄을 함께 지웁니다.</li>
  <li><b>아무나 계정을 얼마든지 만들 수 있는 것</b> — 실제로 로그아웃 상태에서 "패스키 새로 등록하기"를
      반복해 계정이 무한정 늘어나는 것을 직접 겪고 나서 막았습니다. 같은 주소에서 10분 안에 새 계정을
      20번 넘게 시도하면 <code>429</code>로 거절합니다(<code>lib/store.js</code>의
      <code>countRecentNewAccountAttempts</code>, 아직 계정을 만들지 않는 <code>register/options</code>
      단계에서 끊어 헛계정 자체가 안 쌓입니다). 로그인·재확인은 대상이 아닙니다 — 이미 있는 계정 하나에
      매인 패스키로만 되므로 반복해도 새 계정이 생기지 않습니다. ${badge(43)}</li>
</ul>

<p><b>그리고 제출 뒤에 실제 사용 중 발견해서 고친 것이 하나 더 있습니다 — 이게 가장 중요합니다.</b></p>
<div class="warn">
  <p><b>패스키를 지우는 사이에 다른 패스키까지 지워져 계정이 통째로 사라졌습니다.</b>
     삭제 한 번은 <code>첫 DELETE → 403 → 패스키 재확인 → 재시도</code>로 <b>몇 초가 걸립니다.</b>
     그동안 화면이 그대로 살아 있어서, "안 눌렸나?" 하고 다른 줄의 지우기를 누르면 두 요청이
     나란히 나가 <b>패스키가 둘 다 지워졌습니다.</b> 이메일도 비밀번호도 없는 계정이라 그건 곧 계정 소멸입니다.
     실제로 재현했을 때 <code>/api/me</code>가 401, 남은 패스키 0개가 됐습니다.</p>
  <p>통과 기준 검사(28·29·35)는 이걸 잡지 못했습니다 — <b>삭제를 순차로 한 번씩만 시험했기 때문입니다.</b>
     사람이 조급하게 두 번 누르는 상황이 검사에 없었습니다.</p>
  <p><b>고친 방법</b> — 확인을 누르는 즉시 목록 전체를 잠그고(버튼 <code>disabled</code> + 처리기 재진입 차단),
     무엇이 도는 중인지 화면에 바로 적습니다. 그리고 그 상황을 재현하는 검사를 새로 넣었습니다. ${badge(40)} ${badge(41)}</p>
  <p><b>같이 넣은 것</b> — 목록의 각 줄에 <b>"지금 이 기기"</b> 표시를 붙여, 지금 로그인에 쓰고 있는 패스키가
     어느 것인지 눈으로 구분되게 했습니다(세션에 <code>credential_id</code>를 남깁니다). ${badge(42)}</p>
</div>

<p><b>이제 남은 것들입니다. 다 막았다고 적지 않겠습니다.</b></p>
<div class="warn">
  <p><b>1. 비공개 자료는 서버에 그냥 저장됩니다.</b>
     자료 자체를 암호화하지 않았기 때문에, <b>서버의 <code>DATABASE_URL</code>이 새면 전부 읽힙니다.</b>
     패스키가 막는 것은 "바깥에서 들어오는 길"이지 "서버 안에서 보는 눈"이 아닙니다.</p>

  <p><b>2. 계정을 되살릴 방법이 전혀 없습니다.</b>
     기기를 전부 잃거나 마지막 패스키를 지우면 그 계정은 영원히 닫힙니다(검사 35). 일부러 그렇게 두었고 화면에서 경고도 하지만,
     실제 서비스라면 복구 코드 같은 예비 수단이 필요합니다.</p>

  <p><b>3. 실제 기기 두 대로 카드 4를 시연하지 못했습니다.</b>
     아이폰 Face ID로 직접 등록해 저장 위치(iCloud 키체인)까지는 확인했지만, 두 번째로 쓰려던 윈도우 PC가 공용 PC라
     Windows Hello PIN을 새로 만들 수 없었고 블루투스도 켜지지 않아 QR 로그인도 안 됐습니다. 공용 PC에 새 자격 증명을
     만드는 것은 부적절해 시도를 접었습니다. <b>보안 구멍이 아니라 이번 확인의 환경 제약</b>이고, 메커니즘 자체는 가상
     authenticator 검사(26~29·35)가 공개 주소에서 그대로 확인했습니다.</p>

  <p><b>4. 동시 조작 경로를 전부 훑지는 못했습니다.</b>
     위의 삭제 경합은 찾아서 막았지만, 그건 <b>실제로 써 보다가 걸린 하나</b>입니다. 같은 종류(느린 요청이 도는 동안
     화면이 살아 있는 곳)의 다른 경로 — 등록과 삭제를 겹쳐 누르는 경우 같은 것 — 은 검사로 훑지 않았습니다.
     하나를 찾았다는 것은 같은 자리가 더 있을 수 있다는 뜻이라, 없다고 적지 않겠습니다.</p>

  <p><b>5. 속도 제한은 주소(IP) 하나로만 셉니다.</b>
     같은 공유기 뒤 여러 사람이 우연히 몰리면 서로 남의 시도 때문에 막힐 수 있고, 반대로 주소를 계속
     바꿔가며 요청하면 이 제한을 피해갈 수 있습니다. "짧은 시간에 자동으로 수십·수백 개를 찍어 내는
     스크립트"를 막는 데는 충분하지만, 작정하고 주소를 바꿔가며 도는 공격까지 막지는 못합니다.</p>
</div>
