...
<div class="warn">
  <p><b>1. 아무나 계정을 얼마든지 만들 수 있습니다.</b>
     <code>/api/register/options</code>에 <b>요청 수 제한이 없습니다.</b> 비밀번호가 없으니 "비밀번호 무차별 대입"은
     애초에 성립하지 않지만, 등록 자체를 자동으로 반복해 DB를 부풀리는 것은 지금 막지 못합니다.</p>

  <p><b>2. 비공개 자료는 서버에 그냥 저장됩니다.</b>
     자료 자체를 암호화하지 않았기 때문에, <b>서버의 <code>DATABASE_URL</code>이 새면 전부 읽힙니다.</b>
     패스키가 막는 것은 "바깥에서 들어오는 길"이지 "서버 안에서 보는 눈"이 아닙니다.</p>

  <p><b>3. 계정을 되살릴 방법이 전혀 없습니다.</b>
     기기를 전부 잃거나 마지막 패스키를 지우면 그 계정은 영원히 닫힙니다(검사 35). 일부러 그렇게 두었고 화면에서 경고도 하지만,
     실제 서비스라면 복구 코드 같은 예비 수단이 필요합니다.</p>

  <p><b>4. 실제 기기 두 대로 카드 4를 시연하지 못했습니다.</b>
  ...
  <p><b>5. 동시 조작 경로를 전부 훑지는 못했습니다.</b>
