...
function HistoryList({ records, digits, notes }) {
    return (
        <section css={c.panel}>
            <div css={c.panelHead}>
                <h2 css={c.panelTitle}>날짜별 기록</h2>
                <span css={c.panelHint}>하루 한 건 · 매일 {REFERENCE_HOUR}시 값</span>
            </div>
            ...
            {records.length === 0 ? (
                ...
                <ul css={s.list}>
                    {records.map((record) => (
                        <li key={`${record.providerKey}:${record.dateKey}`} css={s.row}>
                            <span css={s.date}>{record.dateKey}</span>
                            <span css={s.value}>
                                {formatValue(record.value, digits)}
                                <small>{record.unit}</small>
                            </span>
                            <span css={s.origin(record.origin)}>{ORIGIN_LABEL[record.origin]}</span>
