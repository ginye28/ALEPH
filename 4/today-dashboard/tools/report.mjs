...
<div class="guide">
  ...
  <h3>무엇을 하나요 (3단계)</h3>
  <ol>
    <li>맨 위 카드에서 <b>값·단위·출처·두 시각·기준 시간대</b>를 확인합니다.</li>
    <li><b>출처</b> 링크를 한 번 누릅니다.</li>
    <li>아래 <b>합성 검사 재생</b>에서 <b>오류 뒤 회복</b>을 누릅니다.</li>
  </ol>

  <h3>무엇이 보이면 통과인가요</h3>
  <ul>
    <li>1단계 — 현재값·단위(${latest.unit})·출처·<code>자료 기준 시각</code>·<code>조회 시각</code>·<code>KST (UTC+9)</code>가 한 화면에 보입니다.</li>
    <li>2단계 — 새 탭에 원자료 JSON이 열리고, 그 안의 <code>current.temperature_2m</code> 값이 화면의 현재값과 같습니다.</li>
    <li>3단계 — 상태가 <code>stale / timeout</code>(마지막 정상값 105 유지)을 거쳐 <b><code>fresh / none</code></b>으로 돌아오고, 일별 행이 1건 → <b>2건</b>이 되며 <code>2026-08-25</code> 행에 값 <code>120</code>이 정확히 한 건 추가됩니다.</li>
  </ul>

  <h3>안 될 때</h3>
  <ul>
    <li>값이 안 보이면 <b>다시 확인</b>을 누릅니다. 출처가 일시적으로 응답하지 않으면 <b>오래된 데이터</b> 표시와 함께 마지막 정상값이 남습니다.</li>
    <li>출처가 안 열리면 브라우저의 새 탭 차단을 해제합니다.</li>
    <li>재생 상태가 꼬이면 <b>reset</b>을 누른 뒤 순서를 다시 누릅니다. 합성 재생은 위쪽 실제 기록을 건드리지 않습니다.</li>
    <li>실제 조회의 장애 상태를 보려면 주소 뒤에 <code>?debug=1</code>, 정상값이 없는 상태를 보려면 <code>?fail=auth</code>를 붙입니다.</li>
  </ul>
</div>
...
${table(["구분", "내용"], [
    ["AI에게 맡긴 일",
     "React·Emotion 구성요소 작성, 날짜 키 변환과 저장소 코드, 참조 어댑터(<code>adapter-reset.example.js</code>)의 상태 전이를 앱으로 이식, 자산 해시 대조와 계약 재생을 확인하는 <code>tools/verify-t04.mjs</code> 작성, 증빙 자동 촬영 스크립트, 보고서 조판."],
    ["내가 판단한 일",
     "어떤 자료를 보여줄지(서울 기온)와 출처 선택. 인증키가 필요 없는 공개 출처를 쓰면 비밀값 관리 문제 자체가 사라진다는 판단. 날짜별 기록의 기준 시각을 매일 09시로 고정해 두 날짜를 같은 조건으로 비교하기로 한 결정. 개정된 요건이 나왔을 때 새 폴더를 파지 않고 <b>이 폴더를 직접 고치기로</b> 한 결정."],
    ["AI 말을 안 들은 일",
     "AI가 처음 설계에서 '이틀에 걸쳐 하루를 기다린 뒤 둘째 기록을 저장한다'고 했지만, 출처가 <code>past_days</code>로 지난 날짜의 실제 관측값을 같은 응답에 함께 준다는 점을 지적해 기다리지 않도록 바꿨습니다. 대신 채워 넣은 기록에는 <code>출처의 지난 기록</code> 표시를 붙여 오늘 직접 본 값과 구분하게 했습니다.<br><br>개정 요건을 처음 읽었을 때 AI가 <b>'백필이 금지됐다'</b>고 단정했는데, 근거를 대라고 하니 계약서(<code>public-contract.json</code>)에는 그런 조항이 없었습니다. '정확히 2건'은 앱 기록이 아니라 <b>플랫폼 과정영수증</b> 수였습니다. AI의 요약을 그대로 믿었으면 멀쩡한 기능을 지울 뻔했습니다."],
])}
