...
import HandoffDoc from "../../components/HandoffDoc/HandoffDoc";
...
import { selectPair } from "../../core/selectPair";
...
function Dashboard() {
    ...
    // 주소를 새로 열면 언제나 기본(최신 2건) 비교부터 보여야 합니다.
    ...
    /**
     * 기록을 바꿔 넣습니다. 새 기록에 없는 날짜가 선택돼 있으면 그 선택은 버립니다.
     * 남겨 두면 기록을 비웠다 다시 채웠을 때 사라졌던 선택이 되살아납니다.
     */
    const applyHistory = useCallback((items) => {
        setHistory(items);
        setBaseKey((key) => (key && items.some((record) => record.dateKey === key) ? key : null));
    }, []);
